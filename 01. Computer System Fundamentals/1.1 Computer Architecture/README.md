...
```
 ...
 └──────┬─────────────┘       │     └──────┬─────────────┘     │
        ...
        ▼                        │            ▼                 │
 ┌──────────────┐         ┌─────▼────┐        ┌───────▼────────┐
 │ 명령어 해석 및 │         │   ALU     │        │   메모리/RAM    │
 │ 실행 흐름 조정 │         │(산술/논리)│        └──────────────┘
 └──────────────┘         └──────────┘

 ※ 주요 흐름:
  - 제어 유닛 → ALU/레지스터에 제어 신호 전달
  - 레지스터 → ALU로 데이터 전달 → 연산 수행
  - 결과는 다시 레지스터나 메모리로 저장
```
...
다음 파트에서는 CPU 외부와 메모리, 입출력 장치가 연결되는 구조를 다루는 **버스와 I/O 시스템**에 대해 이어서 정리할 예정입니다.
